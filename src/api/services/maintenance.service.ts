...
export const maintenanceService = {
  ...
  }): Promise<PreventiveScheduleListResponse> => {
    const response = await apiClient.get('/api/v1/maintenance/preventive-schedules', {
      params: filters,
    });
    return extractData<PreventiveScheduleListResponse>(response.data);
  },

  getPreventiveSchedule: async (scheduleId: number): Promise<PreventiveSchedule> => {
    const response = await apiClient.get(
      `/api/v1/maintenance/preventive-schedules/${scheduleId}`
    );
    return extractData<PreventiveSchedule>(response.data);
  },

  createPreventiveSchedule: async (
  ...
  updatePreventiveSchedule: async (
    scheduleId: number,
  ...
  deletePreventiveSchedule: async (
    scheduleId: number
  ): Promise<{ message: string; id: number }> => {
  ...
  generateWorkOrderFromSchedule: async (
    scheduleId: number
  ): Promise<WorkOrder> => {
    const response = await apiClient.post(
      `/api/v1/maintenance/preventive-schedules/${scheduleId}/generate-work-order`
    );
    return extractData<WorkOrder>(response.data);
  },
  ...
  getInventoryItems: async (filters?: {
    ...
    const response = await apiClient.get('/api/v1/maintenance/inventory', { params: filters });
    return extractData<MaintenanceInventoryItem[]>(response.data);
  },

  getInventoryItem: async (itemId: number): Promise<MaintenanceInventoryItem> => {
    const response = await apiClient.get(`/api/v1/maintenance/inventory/${itemId}`);
    return extractData<MaintenanceInventoryItem>(response.data);
  },

  createInventoryItem: async (data: MaintenanceInventoryCreate): Promise<MaintenanceInventoryItem> => {
    const response = await apiClient.post('/api/v1/maintenance/inventory', data);
    return extractData<MaintenanceInventoryItem>(response.data);
  },

  updateInventoryItem: async (itemId: number, data: MaintenanceInventoryUpdate): Promise<MaintenanceInventoryItem> => {
    const response = await apiClient.patch(`/api/v1/maintenance/inventory/${itemId}`, data);
    return extractData<MaintenanceInventoryItem>(response.data);
  },

  deleteInventoryItem: async (itemId: number): Promise<{ message: string }> => {
    const response = await apiClient.delete(`/api/v1/maintenance/inventory/${itemId}`);
    return extractData<{ message: string }>(response.data);
  },

  adjustInventoryStock: async (
    itemId: number,
    data: StockAdjustment
  ): Promise<MaintenanceInventoryItem> => {
    const response = await apiClient.post(`/api/v1/maintenance/inventory/${itemId}/adjust-stock`, data);
    return extractData<MaintenanceInventoryItem>(response.data);
  },
};
...
export interface PreventiveScheduleCreate {
  name: string;
  description?: string;
  equipment_id?: number;
  location: string;
...
export interface PreventiveScheduleUpdate {
  name?: string;
  description?: string;
  equipment_id?: number;
  location?: string;
...
export interface PreventiveScheduleListResponse {
  total: number;
  overdue_count: number;
  schedules: PreventiveSchedule[];
}

// ===== MAINTENANCE INVENTORY TYPES =====
...
export interface MaintenanceInventoryCreate {
  name: string;
  category?: string;
  stock_level?: number;
  min_stock?: number;
...
export interface MaintenanceInventoryUpdate {
  name?: string;
  category?: string;
  stock_level?: number;
  min_stock?: number;

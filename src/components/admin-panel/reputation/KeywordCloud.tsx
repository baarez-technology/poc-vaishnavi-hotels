    ...
    <div className="bg-white rounded-xl p-6 border border-neutral-200">
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <Hash className={`w-4 h-4 ${isPositive ? 'text-[#4E5840]' : 'text-red-600'}`} />
                    <span className={`font-semibold ${isPositive ? 'text-green-900' : 'text-red-900'}`}>
                      {keyword.word}
                    </span>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <span className={`text-sm ${isPositive ? 'text-[#4E5840]' : 'text-red-700'}`}>
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        ...
        <div className="p-4 bg-red-50 rounded-xl border border-red-200">
          <h5 className="text-sm font-semibold text-red-900 mb-3">Top Negative Words</h5>
          <div className="space-y-2">
            {keywords
              ...
              .map((keyword, index) => (
                <div key={index} className="flex items-center justify-between">
                  <span className="text-sm text-red-800 font-medium">{keyword.word}</span>
                  <span className="text-xs text-red-600 font-semibold">{keyword.count}</span>
                </div>
              ))}
          </div>
        </div>

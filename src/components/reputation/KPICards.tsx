...
import { calculateReputationKPIs } from '../../utils/reputation';
...
export default function KPICards({ reviews }) {
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      suffix: '',
      icon: ThumbsDown,
      bgColor: kpis.negativeReviews > 5 ? 'bg-rose-50' : 'bg-neutral-50',
      iconColor: kpis.negativeReviews > 5 ? 'text-rose-600' : 'text-neutral-600',
      borderColor: kpis.negativeReviews > 5 ? 'border-rose-200' : 'border-neutral-200',
      trend: '-2',
      trendUp: false
    },
    {
      ...
      {kpiCards.map((kpi) => (
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`text-xs font-medium ${kpi.trendUp ? 'text-[#4E5840]' : 'text-rose-600'}`}>
              {kpi.trend}
            </span>
          </div>

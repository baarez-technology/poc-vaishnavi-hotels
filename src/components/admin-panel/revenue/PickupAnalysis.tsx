        ...
        <div className="bg-white p-4 rounded-xl shadow-lg border border-neutral-200">
          ...
          <div className="space-y-1">
            ...
            <p className={`text-sm font-semibold ${
              data.variance >= 0 ? 'text-[#4E5840]' : 'text-red-600'
            }`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className={`p-4 rounded-xl border ${
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`text-sm font-semibold ${
              paceIndicators.overall.status === 'ahead' ? 'text-[#4E5840]' : 'text-red-700'
            }`}>
              Overall Pace
            </span>
          </div>
          <p className={`text-2xl font-bold mb-1 ${
            paceIndicators.overall.status === 'ahead' ? 'text-green-900' : 'text-red-900'
          }`}>
            {paceIndicators.overall.percentage > 0 ? '+' : ''}{paceIndicators.overall.percentage}%
          </p>
          <p className={`text-xs ${
            paceIndicators.overall.status === 'ahead' ? 'text-[#4E5840]' : 'text-red-600'
          }`}>
        ...
        <div className={`p-4 rounded-xl border ${
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`text-sm font-semibold ${
              paceIndicators.shortTerm.status === 'ahead' ? 'text-[#4E5840]' : 'text-red-700'
            }`}>
              Short-term
            </span>
          </div>
          <p className={`text-2xl font-bold mb-1 ${
            paceIndicators.shortTerm.status === 'ahead' ? 'text-green-900' : 'text-red-900'
          }`}>
            {paceIndicators.shortTerm.percentage > 0 ? '+' : ''}{paceIndicators.shortTerm.percentage}%
          </p>
          <p className={`text-xs ${
            paceIndicators.shortTerm.status === 'ahead' ? 'text-[#4E5840]' : 'text-red-600'
          }`}>
        ...
        <div className={`p-4 rounded-xl border ${
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`text-sm font-semibold ${
              paceIndicators.longTerm.status === 'ahead' ? 'text-[#4E5840]' : 'text-red-700'
            }`}>
              Long-term
            </span>
          </div>
          <p className={`text-2xl font-bold mb-1 ${
            paceIndicators.longTerm.status === 'ahead' ? 'text-green-900' : 'text-red-900'
          }`}>
            {paceIndicators.longTerm.percentage}%
          </p>
          <p className={`text-xs ${
            paceIndicators.longTerm.status === 'ahead' ? 'text-[#4E5840]' : 'text-red-600'
          }`}>
